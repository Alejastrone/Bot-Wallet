...
print(f"ADA   = {ada:.5f}")
print(f"usdt ada = {usdt_ada:.2f}")
print(f"AGIX  = {agix:.5f}")
print(f"usdt agix = {usdt_agix:.2f}")
print(f"ALGO  = {algo:.5f}")
print(f"usdt algo = {usdt_algo:.2f}")
print(f"AVAX  = {avax:.5f}")
print(f"usdt avax = {usdt_avax:.2f}")
print(f"BTC   = {btc:.5f}")
print(f"usdt btc = {usdt_btc:.2f}")
print(f"DOT   = {dot:.5f}")
print(f"usdt dot = {usdt_dot:.2f}")
print(f"ETH   = {eth:.5f}")
print(f"usdt eth = {usdt_eth:.2f}")
print(f"FET   = {fet:.5f}")
print(f"usdt fet = {usdt_fet:.2f}")
print(f"FIL   = {fil:.5f}")
print(f"usdt fil = {usdt_fil:.2f}")
print(f"LINK  = {link:.5f}")
print(f"usdt link = {usdt_link:.2f}")
print(f"LTC   = {ltc:.5f}")
print(f"usdt ltc = {usdt_ltc:.2f}")
print(f"MATIC = {matic:.5f}")
print(f"usdt matic = {usdt_matic:.2f}")
print(f"NEO   = {neo:.5f}")
print(f"usdt neo = {usdt_neo:.2f}")
print(f"RNDR  = {rndr:.5f}")
print(f"usdt rndr = {usdt_rndr:.2f}")
print(f"RUNE  = {rune:.5f}")
print(f"usdt rune = {usdt_rune:.2f}")
print(f"SOL   = {sol:.5f}")
print(f"usdt sol = {usdt_sol:.2f}")
print(f"THETA = {theta:.5f}")
print(f"usdt theta = {usdt_theta:.2f}")
print(f"WAVES = {waves:.5f}")
print(f"usdt waves = {usdt_waves:.2f}")
print(f"XLM   = {xlm:.5f}")
print(f"usdt xlm = {usdt_xlm:.2f}")
print(f"XRP   = {xrp:.5f}")
print(f"usdt xrp = {usdt_xrp:.2f}")
print(f"UNI   = {uni:.5f}")
print(f"usdt uni = {usdt_uni:.2f}")

# SISTEMA DE FUCHA
date = datetime.now()
year_month = date.strftime('%m %Y')
with open(f"{apikey.user} cartera.txt", "a") as f:
    f.write(f'<-[{date}]-[{apikey.user}]-\n-[PRECIO BTC = {precio_btc:.2f}]-\n-[TOTAL USDT = {usdt + total_usdt_criptos:.2f}]-\n-[USDT = {usdt:.5f}]-\n-[ADA   = {usdt_ada:.2f} - {ada:.5f}]-\n-[AGIX  = {usdt_agix:.2f} - {agix:.5f}]-\n-[ALGO  = {usdt_algo:.2f} - {algo:.5f}]-\n-[AVAX  = {usdt_avax:.2f} - {avax:.5f}]-\n-[BTC   = {usdt_btc:.2f} - {btc:.5f}]-\n-[DOT   = {usdt_dot:.2f} - {dot:.5f}]-\n-[ETH   = {usdt_eth:.2f} - {eth:.5f}]-\n-[FET   = {usdt_fet:.2f} - {fet:.5f}]-\n-[FIL   = {usdt_fil:.2f} - {fil:.5f}]-\n-[LINK  = {usdt_link:.2f} - {link:.5f}]-\n-[LTC   = {usdt_ltc:.2f} - {ltc:.5f}]-\n-[MATIC = {usdt_matic:.2f} - {matic:.5f}]-\n-[NEO   = {usdt_neo:.2f} - {neo:.5f}]-\n-[RNDR  = {usdt_rndr:.2f} - {rndr:.5f}]-\n-[RUNE  = {usdt_rune:.2f} - {rune:.5f}]-\n-[SOL   = {usdt_sol:.2f} - {sol:.5f}]-\n-[THETA = {usdt_theta:.2f} - {theta:.5f}]-\n-[WAVES = {usdt_waves:.2f} - {waves:.5f}]-\n-[XLM   = {usdt_xlm:.2f} - {xlm:.5f}]-\n-[XRP   = {usdt_xrp:.2f} - {xrp:.5f}]-\n-[UNI   = {usdt_uni:.2f} - {uni:.5f}]-\n--------------------------------------------------\n')